...
- Location: tests\sessionStore.spec.ts:24:5
...
```ts
  ...
  7  | test.describe.serial('Shopping Cart Session Tests', () => {
  ...
  22 | 
  23 | 
  24 | test('Opening stored session', async ({ page }) => {
  25 | 
  26 |     await page.goto('https://rahulshettyacademy.com/client/#/dashboard/cart');
  27 |     const items = page.locator("//li[contains(@class,'items')]");
> 28 |     await expect(items).toHaveCount(2);
     |                         ^ Error: expect(locator).toHaveCount(expected) failed
