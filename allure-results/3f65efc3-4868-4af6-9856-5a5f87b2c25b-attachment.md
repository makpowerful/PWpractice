...
```ts
  ...
  48 | });
  49 | 
  50 | test('Test Right Click', async({page, context})=>{
> 51 |     page.goto("https://www.globalsqa.com/demo-site/draganddrop/");
     |          ^ Error: page.goto: Test ended.
  52 |     page.getByRole('link', { name: 'About' }).first().click({ button: 'right' });
  53 | 
  54 | 
  55 | 
  56 | });
